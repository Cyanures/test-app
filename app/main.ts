...
function createWindow(): BrowserWindow {
  ...
  // Pour customiser le menu : https://www.electronjs.org/docs/latest/api/menu-item
  let defaultMenu = Menu.getApplicationMenu();
  let newMenu = new Menu();
  if(defaultMenu){
    // On boucle sur les items du menu existant et on vire le menu help
    defaultMenu.items
    .filter(x => x.role != 'help')
    .forEach(x => {
      // Si on est pas en production, on garde le menu devtool, sinon on le vire
